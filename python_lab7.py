...
class Player(object):
	...
	def go_to(self, direction):
		...
		self.place = current_place.exit_to(direction)
	...
	def talk_to(self, person):
		...
		"""
		current_place = self.place
		current_people = list(current_place.characters)

		if type(person) != str:
			print('Person has to be a string.')
		elif person in current_people:				# if person is at current location
			print(f"{person} says:", current_place.characters[person].message)
		else:										# if person not at location
			print(f"{person} is not here.")
	...
	def take(self, thing):
		...
		"""
		current_place = self.place
		current_things = list(current_place.things)

		if type(thing) != str:
			print('Thing should be a string.')
		elif thing in current_things:
			print(f"Player takes the {thing}")
			obj = current_place.take(thing)
			self.backpack.append(obj)
		else:
			print(f"{thing} is not here.")
	...
	def check_backpack(self):
		...
		if not self.backpack:
			...
			for item in self.backpack:
				print('   ', item.name, '-', item.description) 	# (for doctests this shows wrong for spacing)
